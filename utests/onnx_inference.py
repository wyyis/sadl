...
parser = argparse.ArgumentParser(prog="onnx_inference and dump", usage="NB: force run on CPU")
...
parser.add_argument("--no_transpose",
        action="store_true",
        default=False,
...
if args.output is not None:
  with open(args.output, 'w') as f:        
        ...
        for input in v:
          ...
          if len(input.shape)<=1 or input.shape[0] != 1:                 
            ...
            if not args.no_transpose:
              if len(input.shape) == 4: input=np.transpose(input,(0,2,3,1)).copy() # nchw to nhwc
        ...
        for o in result:            
            ...
            if not args.no_transpose:
              if len(o.shape) == 4: o=np.transpose(o,(0,2,3,1)).copy() # nchw to nhwc
              if len(o.shape) == 3: o=np.transpose(o,(1,2,0)).copy() # chw to hwc
            for i in o.shape:
                f.write('{} '.format(i))
            f.write('\n')
            for x in np.nditer(o): f.write('{} '.format(x))
            f.write('\n')
