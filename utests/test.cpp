...
template<typename T>
bool readResults(const string &filename, std::vector<sadl::Tensor<T>> &inputs, std::vector<std::vector<float>> &outputs) {
  ...
  for (int i = 0; i < nb_outputs; ++i)
  {
    ...
    dims_out.resize(nb);
    for (auto &x : dims_out) file >> x;
    outputs[i].resize(dims_out.nbElements());
    for (auto &x : outputs[i]) file >> x;
  }
...
template<typename T>
bool checkResults(const std::vector<float> &gt, const sadl::Tensor<T> &test, double abs_tol) {
  double max_a = 0.;
  int nb_e = 0;
  double max_fabs = 0.;
  double mae = 0.;
...
template <typename T>
int infer(const std::string &filename_results, const std::string &filename, double max_e) {
  ...
  if (verbose>1) {
    for(const auto &t: inputs) cout<<"Input "<<t<<endl;
  }
  ...
  for(int i=0;i<(int)dim_outs.size();++i) {
    if (dim_outs[i].size()+1==model.result(i).dims().size()) {
      ...
      dim_outs[i][0]=1;
      for(int k=0;k<d.size();++k) dim_outs[i][k+1]=d[k];
    }
  ...
  for (int i = 0; i < (int)outputs.size(); ++i) {
      if (verbose > 1) {
        ...
        for (int k = 0; k < t.size(); ++k) t[k] = outputs[i][k];
        cout << "[INFO] output file " << i << "\n" << t << endl;
      }
      if (verbose > 1) cout << "[INFO] output model " << i << "\n" << model.result(i) << endl;
